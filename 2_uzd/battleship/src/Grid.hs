...
letterToNumber [letter] = ord letter - 64
letterToNumber _ = 0;
...
flattenMessageReversed Message {coord = Just (coordX, coordY), prev = Nothing} = ([], [coordY, coordX])
flattenMessageReversed Message {coord = Just (coordX, coordY), result = Just prevRes, prev = Just prevMessage} =
...
flattenMessageReversed Message {coord = Nothing, result = Just prevRes, prev = Just prevMessage} =
  flattenMessageReversed Message {coord = Just ("0", "0"), result = Just prevRes, prev = Just prevMessage}
flattenMessageReversed _ = ([], ["0", "0"])
